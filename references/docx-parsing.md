...
- 默认将每个关键词的普通单词规范为小写；第一个关键词开头可因句首大写
- 双栏 HTML 与单栏 HTML 必须使用完全一致的关键词字符串
- 仅在专有名词、标准基因符号、数据库缩写等大小写具有明确语义时保留原样；若无法确认，默认使用小写
- 若用户或目标期刊明确指定每组关键词首词首字母大写，按该要求覆盖默认规则，并同步双栏、单栏和本地预览；禁止在未说明规则时出现同一行内部分关键词首字母大写、部分全小写的混排
...
- [ ] 作者已提取
- [ ] 摘要已提取
- [ ] 关键词已提取并规范为目标格式（逗号分隔；默认普通词小写，或按用户/期刊要求首词首字母大写）
- [ ] 简短标题已向用户以可复制文本明确展示
- [ ] 图片数量已统计
- [ ] 参考文献已提取
